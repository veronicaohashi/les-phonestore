...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    	<h1 class="h2">Alterar Celular: <%= phone.getId() %></h1>  	
	...
	<input type="hidden" name="id" value="<%=phone.getId()%>" /> 
			    	...
			    	<label for="cbConnection">Conectividade</label>			    	
			  		<%
						...
						for(DomainEntity c : connectionList){
							...
							/*for(ConnectionType ct : phone.getConnectionType()){
								if(ct.getId() == c.getId())
									out.println("<option value='" + ct.getId() +"' selected>"+ ct.getDescription() +"</option>");
								else
									out.println("<option value='" + ((ConnectionType)c).getId() + "'>" +  ((ConnectionType)c).getDescription()  +"</option>");
		...
		<input type="submit" name="action" value="UPDATE" class="btn btn-sm btn-outline-secondary"/>
	</form>
</main>
